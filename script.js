...
class SnakeRace{
	...
	iniciarReconocimientoVoz = () => {
		// Comprobar si el navegador admite SpeechRecognition
	 ...
	 this.reconocimiento.lang = 'es-ES'; // Configuración para español
	 ...
	 // Evento para procesar el resultado del reconocimiento de voz
	...
	teclaPulsada = (evento) => {
	 ...
	 // Método para procesar los comandos de voz
	 procesarComandoVoz = (comando) => {
		if (comando === "arriba" && this.cabeza.dir !== 2) {
		  this.cabeza.giro = 0;
		} else if (comando === "abajo" && this.cabeza.dir !== 0) {
		  this.cabeza.giro = 2;
		} else if (comando === "izquierda" && this.cabeza.dir !== 1) {
		  this.cabeza.giro = 3;
		} else if (comando === "derecha" && this.cabeza.dir !== 3) {
		  this.cabeza.giro = 1;
		}
	  }
	actualizar = () => {
	...
	finalizar = (tipo = 1) => {
		...
		switch(tipo) {
			case 0: // Colisión con bordes
				...
				mensaje = 'Has chocado con el borde del circuito. Intenta mantener el control en los límites.';
				break;
			case 1: // Colisión con barrera
				img.src = this.imgChoque2.src;
				mensaje = 'Has colisionado con una barrera. Mantente alerta y evita obstáculos.';
				break;
			case 2: // Abandono
				img.src = this.imgAbandono.src;
				mensaje = 'Se ha terminado el combustible o los neumáticos. Recuerda la importancia de la gestión de recursos.';
				break;
		}
		
		divTexto.textContent = mensaje;
		this.divDialogo.style.display = 'block';
		
		// Llamar a leerEnVozAlta con el mensaje adecuado
		this.leerEnVozAlta(mensaje);
	}
